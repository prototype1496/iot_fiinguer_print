...
if(isset($_GET['student_id'])){
    ...
    $row = SuperModel:: get_checkout_student_by_id($student_id);
    ...
    if(is_null($row) || empty($row) || !isset($row)){
        echo "<script>
            ...
            function(){
               ...
               $.jnoty('Check out not successful please make sure we have checked in and not already checked out', {
            sticky: false,
            header: 'Info',
            theme: 'jnoty-info',
            close: function() {window.location.replace('/stms/view/teacher/CloseRegister.php')},
            });
            });
            </script>";
    }else{
        if(SuperModel::close_report_record($row['StudentReportDetailsPublicID'])){
            SMSNotification::sendSMS("Greetings " . $row['StudentName'] . " has left");
            echo "<script>
            $(document).ready(

            function(){

               $.jnoty('Successfully Checked Out :)', {
            sticky: false,
            header: 'Notification',
            theme: 'jnoty-success',
            close: function() {window.location.replace('/stms/view/teacher/CloseRegister.php')},
            ...
            function(){
               ...
               $.jnoty('The was an issue when checking out please try again', {
            sticky: false,
            header: 'Error',
            theme: 'jnoty-danger',
            close: function() {window.location.replace('/stms/view/teacher/CloseRegister.php')},
                        ...
                        <div class="card-block">
                            <div class="row align-items-center">
                                <div class="col">
                                    ...
                                    <p class=" h3 m-b-10">CHECK OUT ATTENDANCE </p>
                                </div>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    //  The script below is going to be used to process the booking of a patinet that is in the modle
        ...
        function movetoscan(){
            window.location.href = "../../java_programs/verifcation/FingerPintVerification2.php?";
